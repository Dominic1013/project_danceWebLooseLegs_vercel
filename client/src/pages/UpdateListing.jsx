...
export default function UpdateListing() {
  ...
  // useEffect to get input data, useEffect不能使用async，但可以讓裡面的函數去使用
  useEffect(() => {
  ...
  const handleRemoveImage = async (index) => {
    if (fireBaseUser) {
      ...
      try {
        ...
        window.alert("delete action failed, please Sign in again.");
        console.error("Firebase's delete action is failed:", error.message);
      }
    } else {
      //fireBase用戶未登入
      window.alert("Your authentication is rejected, please Sign in again.");
